<template>
    ...
    <div class="shadow overflow-hidden sm:rounded-lg">
      <div class="px-4 py-5 sm:p-6">
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-xl font-bold">Admin: All AWS Instances</h2>
          <button
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th scope="col" class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Owner</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="instance in instances" :key="instance.id">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ instance.createdBy }}</td>
        ...
        <div v-if="!loading && instances.length === 0" class="text-center py-8">
          ...
          <p class="mt-1 text-sm text-gray-500">There are no AWS instances in the system.</p>
        </div>
      </div>
    </div>
        ...
        <div class="px-6 py-4">
          <form @submit.prevent="saveInstance">
            ...
            <div class="mb-4">
              ...
              <input
                v-model.number="currentInstance.createdBy"
                type="number"
                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500"
                required
              />
  ...
  <div v-else class="mt-8">
    <div class="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
      ...
      <p class="mt-2 text-red-700">You do not have permission to view this page.</p>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async loadInstances() {
      ...
      try {
        const data = await awsInstanceService.getAdminInstances()
        this.instances = data
      } catch (error) {
    ...
    async confirmDelete() {
      ...
      try {
        await awsInstanceService.deleteAdminInstance(this.instanceToDelete.id)
    ...
    async saveInstance() {
      try {
        if (this.showEditModal) {
          ...
          await awsInstanceService.updateAdminInstance(this.editingInstanceId, this.currentInstance)
        } else {
          // Create new instance
          await awsInstanceService.createAdminInstance(this.currentInstance)
        }
